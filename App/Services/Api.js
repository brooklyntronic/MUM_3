...
const create = (baseURL = Utilities.baseUrl) => {
  ...
  //Polls every three seconds until receiving a status of 200 or 503
  const pollTranscoder = (jobId) => Utilities.poll(()=>api.post('matchups/pollTranscoder', {jobId}), 3000)
  ...
  const searchMatchups = (searchTerm, myList) => api.post('searchMatchups', {searchTerm: searchTerm, myList: myList})
  ...
  const searchMatches = (searchTerm) => api.post('bySearch', {searchTerm})
